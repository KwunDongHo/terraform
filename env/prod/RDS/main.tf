...
# 데이터베이스 서브넷 그룹 생성
resource "aws_db_subnet_group" "sportslink_subnet_group" {
  name       = "sportslink-subnet-group"
...
module "RDS" {
  ...
  identifier                          = "sportslink-rds" # 식별이름 : 알파벳 소문자, 하이픈만 사용가능
  engine                              = "mysql"
  ...
  db_subnet_group_name   = aws_db_subnet_group.sportslink_subnet_group.name  
  subnet_ids             = data.terraform_remote_state.vpc.outputs.database_subnet_ids
